...
parser.add_argument('--eps', type=float, default=150)
...
def add_stealthy_attack(input_latent, eps):
    ...
    cumulative_sum = 2*torch.cumsum((2*sort_val)**2, dim=0)
    # get largest k
    largest_k = 0
...
def add_min_distortion_attack(input_latent, eps):
    ...
    return input_latent_flat.reshape(input_latent.shape).to('cuda:1')
...
for i in tqdm(range(test_num)):
    ...
    print(torch.sum((reversed_latents - reversed_latents_attack)**2))
    orig_image, _, _ = generate(prompt=current_prompt,
